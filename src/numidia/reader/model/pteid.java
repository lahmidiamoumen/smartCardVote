...
public class pteid {
   ...
   public static PTEID_EId GetID() throws PteidException{
       try {
            return  idCard.getID();
        } catch (Exception ex) {
            throw new PteidException();
...
//
            ...
            try {
                ...
                for (long pinIdx = 0; pinIdx < pins.count(); pinIdx++) {
                    ...
                    if (pin.getPinRef() == pinId) {
                        boolean rc = pin.verifyPin(string, tries_left, false);
                        if (!rc)
                        {
    ...
    public static pteidlib.PTEID_Pin[] GetPINs() throws PteidException {
        pteidlib.PTEID_Pin[] pinArray = null;
        int currentId;

        if (readerContext != null) {
            try {
                PTEID_Pins pins = idCard.getPins();
                pinArray = new pteidlib.PTEID_Pin[PTEID_PIN_COUNT];
                for (int pinIdx = 0; pinIdx < pins.count(); pinIdx++) {
                    PTEID_Pin pin = pins.getPinByNumber(pinIdx);
                    if (pin.getId() == 1 || pin.getId() == 2 || pin.getId() == 3) {
                        currentId = (int)pin.getId()-1;
                        pinArray[currentId] = new pteidlib.PTEID_Pin();
...
//    }
    ...
    public static int UnblockPIN(long pinId, String puk, String newPin) throws PteidException {

       return UnblockPIN_Ext(pinId, puk, newPin, UNBLOCK_FLAG_NEW_PIN);
    }

    public static int UnblockPIN_Ext(long pinId, String puk, String newPin, int flags) throws PteidException {
        PTEID_ulwrapper tries_left = new PTEID_ulwrapper(-1);
